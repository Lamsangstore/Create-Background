            ...
            <React.Fragment key={i}>
              <div className="flex flex-col items-center gap-1.5 shrink-0 w-20 sm:w-28">
                ...
                <div className="text-center leading-tight">
                  <div className={`text-[9px] uppercase tracking-widest font-bold ${isDone || isActive ? 'text-gold-dark' : 'text-subtle'}`}>
                    STEP {i + 1}
                  </div>
                  <div className={`text-[11px] font-semibold ${isDone || isActive ? 'text-ink' : 'text-muted'}`}>
                    {s.title}
                  </div>
                </div>
              </div>
...
export const StepHeader: React.FC<{ step: number; title: string; subtitle?: string }> = ({
  ...
  return (
    <div className="flex items-center gap-3.5 px-1">
      <div className="w-9 h-9 rounded-full bg-gradient-to-b from-[#d3a866] to-[#c79a5b] text-white flex items-center justify-center text-sm font-bold shadow-[0_6px_16px_-6px_rgba(199,154,91,0.6)] shrink-0">
        {step}
      </div>
      <div>
        <span className="text-[10px] uppercase tracking-[0.25em] font-bold text-gold-dark">STEP {step}</span>
        <h2 className="text-[15px] font-semibold text-ink leading-tight">{title}</h2>
        {subtitle && <p className="text-[12px] text-muted leading-snug mt-0.5">{subtitle}</p>}
      </div>
    </div>
  );
};
