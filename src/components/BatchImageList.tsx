...
import { Download, Trash2, RefreshCw, Eye, Sparkles, CheckCircle2, AlertTriangle, FileArchive, Loader2, FileText } from 'lucide-react';
...
export const BatchImageList: React.FC<BatchImageListProps> = ({
  ...
  // Live preview of the resulting file name.
  const previewBase = productName.trim()
    ? sanitizeFileName(productName)
    : sanitizeFileName(items[0]?.name || 'product');
  const fileNamePreview = `${previewBase}${items.length > 1 ? '_1' : ''}.png`;
              ...
              <div className="flex items-center gap-1.5 pt-2">
                ...
                {item.status === 'completed' && (
                  <button
                    ...
                    className="p-2 rounded-lg text-muted hover:text-gold hover:bg-cream-2 border border-transparent hover:border-line transition-colors cursor-pointer"
